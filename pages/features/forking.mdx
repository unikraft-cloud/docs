...
This works like [`fork(2)`](https://man7.org/linux/man-pages/man2/fork.2.html) in a POSIX process.
...
The platform serializes fork operations.
Only one fork can run at a time.
If a second fork request arrives while one runs, the platform queues it.
If the queue fills up, the write returns `EBUSY`.

The platform processes queued requests in order once the current fork completes.
...
The platform notifies each one when the child exits.
...
- Children have [`delete-on-stop`](/platform/instances#delete-on-stop) enabled automatically.
