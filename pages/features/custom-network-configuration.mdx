...
The relay feature lets you redirect all network traffic from and to a network interface to another network interface.
In practice, this means you can set up a gateway or proxy VM and configure its network interface as the relay for another VM's network interface.

This is useful in sandbox environments - as a gateway or proxy can inspect, filter, and inject data into the sandbox's traffic.
In this setup, the sandbox doesn't need unfiltered network access or access to secrets.
