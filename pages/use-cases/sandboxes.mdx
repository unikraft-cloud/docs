...
// `await using` deletes the sandbox when this scope ends, however it ends.
await using sandbox = await Sandbox.create();

const { stdout } = await sandbox.exec("echo hello");
await sandbox.writeFile("/work/input.txt", "...");
```

Commands, standard input, signals and the filesystem are all covered.
The [Python](/sdks/python) and [Go](/sdks/go) SDKs drive the instances themselves.
:::
