...
Because a ROM has no `runtime` to take architectures from, its `Kraftfile` must name them under `targets`, or you pass `--arch` on the command line.
To package them you can use the CLI:

<CodeTabs syncKey="cli">

```bash title="unikraft"
# make sure to specify the rom path and the build targets in the Kraftfile
# ...
# targets:
# - kraftcloud/x86_64
# roms:
