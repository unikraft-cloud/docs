...
[Instances](/platform/instances) can now carry user-defined [annotations](/features/annotations), which you can also include in the instance log output.
...
You can now hotplug new [volumes](/platform/volumes) to instances during [scale-to-zero](/features/scale-to-zero) cycles and add volumes to instances created from [templates](/features/on-demand-templates).
...
Operators can continue to export [metrics](/platform/metrics) for instances that the platform still retains after removal, which is useful for debugging and accounting.
