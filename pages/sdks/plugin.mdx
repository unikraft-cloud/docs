...
In Go the envelope type is `platform.Response[T]` from `unikraft.com/cloud/sdk/platform`, which the [Go SDK](/sdks/go) uses as well.
...
* [Go SDK](/sdks/go): the client library for the platform API itself.
* [JavaScript SDK](/sdks/js): the same platform API from JavaScript and TypeScript.
* [Python SDK](/sdks/python): the same platform API from Python.
