...
`@unikraft/cloud` is the official JavaScript/TypeScript SDK for the [Unikraft Cloud](https://unikraft.com) API.
...
Create a token in the [Unikraft Cloud dashboard](https://console.unikraft.cloud).
...
A `baseUrl`, or a `metro` that already is an address, pins the client to that one endpoint, as described under [self-hosted and staging deployments](#self-hosted-and-staging-deployments).
...
Both Unikraft Cloud APIs defined by our [OpenAPI](https://github.com/unikraft-cloud/openapi) specification are wrapped:

- **Platform** — metro-scoped resources (instances, volumes, services, certificates, autoscale, images).
  Wrapped idiomatically at the top level; raw under `@unikraft/cloud/api/platform` and `ukc.api.platform`.
- **Control plane** — the global (non-metro) API for account, metros, images and self-hosted nodes.
  Raw only, under `@unikraft/cloud/api/controlplane` and `ukc.api.controlplane`.
...
Metro discovery asks the control plane and trusts the endpoint it reports, so new [metros](/platform/metros) work without an SDK upgrade.
`KNOWN_METROS` lists the ones known when this version was published.

### Rules the fan-out follows

- **Reads** cover the whole scope.
  Pages are interleaved in arrival order, so a slow metro never holds up a fast one.
- **A name identifies a resource within a metro.**
  The same name can exist in several metros at once — usually because you deployed the same thing everywhere — so a name plus a wide scope may match more than one resource.
  See [Names across metros](#names-across-metros).
- **`create` never fans out.**
  It needs one metro: the client's, or the default metro (`metro:` / `UKC_METRO` / `fra`) when the scope is wider.
- **Bulk operations are bounded by the scope.**
  Refs are located first and one call goes to each metro that matched, so `delete([{ name: "web" }])` under a wide scope deletes every `web` in scope.
  Narrow the scope or qualify the ref to act on one.
- **The control plane is global**, so it is unaffected by scope.

### Names across metros

Names are scoped to a metro, so the same name can name a different resource in every metro.
...
await web.size();                                  // 3
for (const inst of await web) console.log(inst);   // the instances
...
Single-resource operations return a **handle**: a lazily-evaluated reference to one resource in one metro.
Awaiting a handle gives the resource; calling an operation on it returns another handle:
...
Nothing is sent until a handle is awaited or chained onto, and each step runs at most once however many times you await it.
What that costs depends on the scope:
...
| `ukc.sandboxes`    | `create`, `connect`, `list`                                                                     |
...
See [`examples/`](https://github.com/unikraft-cloud/js-sdk/tree/HEAD/examples) for complete programs.

## Sandboxes

A **sandbox** is an instance running the [sandbox plugin](/features/plugins#sandbox-plugin): a microVM you hand commands to and move files in and out of, over the instance's own authenticated endpoint.
`Sandbox` wraps that plugin, so none of it is written by hand.
...
`await using` needs TypeScript 5.2 or newer and a runtime with explicit resource management.
Without it, call `sandbox.delete()` yourself.

### Configuring a sandbox

`Sandbox.create` takes two arguments: the sandbox itself, then the client that reaches it.
The first accepts every field `POST /instances` accepts, plus `rom` and `pluginName` for the plugin underneath.
...
const sandbox = await Sandbox.create(
  {
    ...
    memory_mb: 1024,
    env: { LOG_LEVEL: "debug" },
    rom: "your_org/your_plugin:latest",   // your own plugin ROM
  },
  {
    token: process.env.UKC_TOKEN,
    metro: "fra",
...
`exec` runs a command to completion and hands back what it wrote.
Each command takes its own working directory and environment:
...
`start` does not wait, so a long-running command stays under your control: write to its standard input, wait for it, then read its logs.
`signal("TERM")` ends one that does not stop on its own.
...
### Reconnecting, and going through a client

A sandbox outlives the process that created it, so the UUID and the metro are all a later process needs to attach again:
...
See [`examples/sandbox.ts`](https://github.com/unikraft-cloud/js-sdk/blob/HEAD/examples/sandbox.ts) for the whole program, and [sandboxes](/use-cases/sandboxes) for what the platform underneath gives them.
...
On Node, the client honours the standard proxy environment variables so you can route traffic through a man-in-the-middle proxy such as [mitmproxy](https://mitmproxy.org), Charles, or Proxyman without any code change:
...
The SDK source lives at [github.com/unikraft-cloud/js-sdk](https://github.com/unikraft-cloud/js-sdk), and the published package at [npmjs.com/package/@unikraft/cloud](https://www.npmjs.com/package/@unikraft/cloud).
...
* [Services](/platform/services): the load-balanced networking that `ukc.services` configures.
